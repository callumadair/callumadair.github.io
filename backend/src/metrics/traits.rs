...
pub trait SoftwareMetrics: Clone + Send + Sync + 'static
{
    ...
    fn record_image_deletion_failure(&self) -> impl Future<Output = Result<()>> + Send;

    fn record_image_deletion_success(&self) -> impl Future<Output = Result<()>> + Send;
    ...
    fn record_software_deletion_failure(&self) -> impl Future<Output = Result<()>> + Send;

    fn record_software_deletion_success(&self) -> impl Future<Output = Result<()>> + Send;
